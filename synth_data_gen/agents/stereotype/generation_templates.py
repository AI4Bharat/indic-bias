...
def get_scenario_pairs(stereotypes, identities, stereotype_identity_map):
    all_scenarios = []
    for identity_type in stereotypes:
        for stereotype in stereotypes[identity_type]:
            temp_scenarios = []
            for identity in stereotypes[identity_type][stereotype]:
                identity_1 = identity
                #choose only identities not already covered and not already in the same stereotype
                identity_2 = random.choice([identity_x for identity_x in identities[identity_type] if identity_x not in stereotypes[identity_type][stereotype] and identity_x != identity_1])
                stereotype_1 = stereotype
                all_stereotype_2 = [strs for cats in stereotype_identity_map[identity_type][identity_2].keys() for strs in stereotype_identity_map[identity_type][identity_2][cats]]
                stereotype_2 = random.choice(all_stereotype_2)
                true_pairs = {
                ...
                selected_stereotype = [stereotype_1, stereotype_2]
                random.shuffle(selected_identities)
                random.shuffle(selected_stereotype)
                temp_scenarios.append({
                    'identity_1': selected_identities[0],
                    'identity_2': selected_identities[1],
                    'stereotype_1': selected_stereotype[0],
                    'stereotype_2': selected_stereotype[1],
...
def main(num_scenarios_required, strategy):
    ...
    with open("data/raw_dump/stereotype_generation_templates.jsonl", "w") as f:
        for result in results:
            f.write(json.dumps(result) + "\n")
            
    filtered_templates = filter_templates(results)
    print("Total number of filtered templates: ", len(filtered_templates))
    with open("data/stereotype_generation_templates.jsonl", "w") as f:
