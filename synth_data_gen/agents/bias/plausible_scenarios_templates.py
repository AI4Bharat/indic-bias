...
from synth_data_gen.prompts.bias.plausible_scenarios import plausible_scenario_agent_prompt
from rich.logging import RichHandler
from joblib import Parallel, delayed
from config import AZURE_OPENAI_BASE_URL, AZURE_OPENAI_API_KEY, AZURE_OPENAI_API_VERSION


# topics_file = "data/bias_plausible_scenario_topics.jsonl"
...
def main(num_topics_required, strategy):
    ...
    prompt_template = plausible_scenario_agent_prompt
    ...
    with open("data/raw_dump/bias_indic_plausible_scenario_templates.jsonl", "w") as f:
        for result in results:
            f.write(json.dumps(result) + "\n")
            
    filtered_templates = filter_templates(results)
    print("Total number of filtered templates: ", len(filtered_templates))
    with open("data/bias_indic_plausible_scenario_templates.jsonl", "w") as f:
