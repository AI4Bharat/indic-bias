...
from prompts.bias.topics import generation_topics_agent_prompt
...
def main(num_concepts_required, strategy):
    ...
    prompt_template = generation_topics_agent_prompt
    ...
    with open("data/raw_dump/bias_generation_topics.jsonl", "w") as f:
        for result in results:
            f.write(json.dumps(result) + "\n")
            
    filtered_topics = filter_topics(results)
    print("Total number of filtered topics: ", len(filtered_topics))
    with open("data/bias_generation_topics.jsonl", "w") as f:
